...
float fbm(vec2 p){
    ...
    for(int i=0;i<5;i++){
        ...
        amp*=.73;// reduce amplitude (self-similarity) :contentReference[oaicite:1]{index=1}
    }
    return v;
}
...
void main(){
    ...
    vec2 uv=(gl_FragCoord.xy/-3.-u_resolution.xy)
